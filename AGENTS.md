# AGENTS.md — market-data-lake-worker

> 通用 AI Agent 项目记忆（[agents.md](https://agents.md/) 标准）。Claude Code 用户请同时参考 `CLAUDE.md`；完整 PRD 见 `agent-spec.md`。

## 项目用途

跑在 Cloudflare Workers 上、**S3-only**（禁用 R2 binding）、**多粒度**（日/小时/分钟可配置）、自带 **Cloudflare Access 鉴权管理后台**的全球股市行情爬虫。定时抓美股/港股/A股的 OHLCV 行情，通过标准 S3 协议合并落地到对象存储，元数据用 D1 管理。
...
├── AGENTS.md                # 本文件
├── CLAUDE.md                # Claude Code 专属记忆（与本文件共享红线）
├── .claude/agents/market-data-worker.md  # Claude Code 子 Agent
└── src/
...
## Cloudflare Access 配置（一次性）

详见 `agent-spec.md` 的「Cloudflare 侧手动配置」节。要点：

1. 启用 Zero Trust → Access → Add **Self-hosted Application**
2. Application domain = Worker 自定义域名（**必须橙云代理**）
3. Identity providers 选 `One-time PIN`
4. Policy → Allow Emails 白名单
5. Application 的 **Audience Tag** 配进 `wrangler.jsonc` 的 `ACCESS_AUD`
