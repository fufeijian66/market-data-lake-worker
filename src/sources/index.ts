...
import { fetchEastmoney, fetchEastmoneyHK } from './eastmoney';

/**
 * 选择数据源：
 *   - US：Yahoo Finance
 *   - HK / CN 全粒度：东方财富（Yahoo 近期开启 403 反爬，HK 改走东方财富）
 *
 ...
 *
 * 备注：src/sources/sina.ts 仍保留作为可切换的备用源；CN/HK 调度统一走东方财富。
 */
export async function fetchOHLCV(
  ...
  if (market === 'US') return fetchYahoo(ticker, interval, dataEndAt);
  if (market === 'HK') return fetchEastmoneyHK(ticker, interval, dataEndAt);
